...
const AuthModal: FC<AuthModalProps> = ({ show = false, onClose = () => null }) => {
  const [disabled, setDisabled] = useState(false);
  const [showConfirm, setConfirm] = useState(false);
  const [showSignIn, setShowSignIn] = useState(false);

  const signInWithEmail: FormikConfig<{ email: string }>['onSubmit'] = async ({ email }) => {
            ...
            <div className='inline-block w-full my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl sm:rounded-md max-w-md relative'>
              ...
              <div className='py-12'>
                <div className='px-4 sm:px-12'>
                  ...
                  <Dialog.Title as='h3' className='mt-6 font-bold text-lg sm:text-2xl text-center'>
